...
function init() {
  loadImages(function () {
    var button = document.getElementById('gen-map-button');
...
function generate() {
  ...
  var mapDef;
  switch ($("[name='game-type']:checked").val()) {
    case 'expanded':
      mapDef = expandedMap;
      break;
    default:
  ...
  console.log('Map definition:', mapDef);
  catanMap.defineMap(mapDef);
  catanMap.generate();
  catanMap.resize();
  catanMap.draw();
...
HexTile.prototype.drawNumber = function () {
  ...
  drawingContext.arc(
    this.xCenter,
    this.yCenter,
    Math.abs(0.375 * size), // Ensure the radius is positive
    0,
    2 * Math.PI,
    false
  );
